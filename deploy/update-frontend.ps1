...
docker run -d --name app_mesaayuda --restart unless-stopped -p 8010:80 surgicorp/app_mesaayuda:latest | Out-Null

Write-Host "==> verificar" -ForegroundColor Cyan
$code = curl.exe -s -o $null -w "%{http_code}" http://localhost:8010/
