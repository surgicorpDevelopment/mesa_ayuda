...
# Cola helpdesk: abiertos sin asignar (cualquier área)
Q_COLA_SIN_ASIGNAR = Q(asignado_a__isnull=True) & ~Q(
    estado__in=[EstadoTicket.RESUELTO, EstadoTicket.CERRADO]
)
...
def visible_tickets(user):
    """Desarrollador+ ve los suyos, los de su área y la cola sin asignar; el resto solo los suyos."""
    qs = GP_Ticket.objects.select_related('reportado_por', 'asignado_a', 'proyecto').all()
    if is_gestor(user):
        return qs
    area_id = get_user_area_id(user)
    if is_lider(user):
        q = Q(reportado_por=user) | Q(asignado_a=user) | Q_COLA_SIN_ASIGNAR
        if area_id is not None:
            q |= Q(area_id=area_id)
        return qs.filter(q)
    if is_desarrollador(user):
        q = Q(asignado_a=user) | Q(reportado_por=user) | Q_COLA_SIN_ASIGNAR
        if area_id is not None:
            q |= Q(area_id=area_id)
        return qs.filter(q)
